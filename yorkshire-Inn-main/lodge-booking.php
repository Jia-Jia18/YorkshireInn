...
----------------------------------------
Author: Chris Pegoli
File: lodge-booking.php
Version: 1.0
Release Notes: Initial Release.
----------------------------------------
Purpose: Handless the session variables from the checkIn, checkOut, numGuest fields as well as 
all the posted field data from blue-payment.php page. Calculates neccessary billing math, 
...
$grandTotal4 = $_SESSION['grandTotal4'];
...
$sql = "INSERT INTO reservation(BookingDate, CheckIn, CheckOut, NumGuests, SpecialReq, TotalPayment, TotalDeposit, RoomID, TravelerID, PaymentID) 
        VALUES ('$insertTime', '$checkIn', '$checkOut', '$numGuests', '$SpecialReq', '$grandTotal4', '$deposit4', '$RoomID', '$travID', '$payID')";
$result = mysqli_query($conn, $sql);

// SQL query blocks the availability of the room between the check in and check out dates
$sql = "UPDATE isAvailable SET isAvailableBool4 = 0 WHERE isAvailableDate BETWEEN '" . $checkIn . "' AND '" . $checkOut . "'";
$result = mysqli_query($conn, $sql);
...
// Redirects the customer to lodge-confirmation.php and passes the reservation ID to the page as well as puts it in the URL
header("Location: lodge-confirmation.php?resID=$resID");
?>
